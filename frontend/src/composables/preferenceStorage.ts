 ...
 *
 * Storage layout: TreeRAG_${userId}_${suffix}, where userId is the active
 ...
 *   - Legacy un-namespaced keys (TreeRAG_${suffix}) from earlier branch
...
export function safeSetItem(key: string, value: string): void {
  try {
    ...
    console.warn(`[TreeRAG] failed to persist preference "${key}":`, err)
  }
}
...
export function userKey(suffix: string): string {
  return `TreeRAG_${readUserId()}_${suffix}`
}
...
export function migratePreferencesIntoUser(): void {
  ...
  for (const suffix of PREFERENCE_SUFFIXES) {
    const target = `TreeRAG_${userId}_${suffix}`
    const targetExists = safeGetItem(target) !== null

    const anonKey = `TreeRAG_anon_${suffix}`
    const legacyKey = `TreeRAG_${suffix}`
