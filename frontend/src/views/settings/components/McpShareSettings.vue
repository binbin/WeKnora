      ...
      <div class="share-panel-header-row">
        <div class="share-panel-titlewrap">
          ...
          <t-popup
            ...
            trigger="hover"
            overlay-class-name="share-hint-popup-overlay"
            :overlay-inner-style="shareHintPopupInnerStyle"
          >
            <button
              type="button"
              class="share-hint-trigger-btn"
              :aria-label="$t('mcpServiceDialog.share.hintTitle')"
              :title="$t('mcpServiceDialog.share.hintTitle')"
            >
              <t-icon name="info-circle" size="16px" />
            </button>
            <template #content>
              <div class="share-hint-popover">
                <p class="share-hint-title">{{ $t('mcpServiceDialog.share.hintTitle') }}</p>
                <p class="share-hint-desc">{{ $t('mcpServiceDialog.share.tip1') }}</p>
                <p class="share-hint-desc">{{ $t('mcpServiceDialog.share.tip2') }}</p>
              </div>
            </template>
          </t-popup>
        </div>
      </div>
      <p class="share-panel-desc">{{ $t('mcpServiceDialog.share.description') }}</p>
    </div>
                ...
                <div class="org-upgrade-fields">
                  ...
                  <div class="org-upgrade-field org-upgrade-field--last">
                    ...
                    <t-select v-model="selectedPermission" size="medium">
                      <t-option
                        value="viewer"
                        :label="$t('organization.share.permissionReadonly')"
                      />
                      <t-option
                        value="editor"
                        :label="$t('organization.share.permissionEditable')"
                      />
                    </t-select>
                    <p class="member-form-hint">
        ...
        >
          ...
          <template #actions="{ row }">
            <div v-if="canShare" class="share-table-actions">
              <t-popconfirm
                :content="
                  $t('knowledgeEditor.share.unshareConfirm', {
                    name: row.organization_name,
                  })
                "
                ...
                placement="left"
                @confirm="handleUnshare(row)"
              >
                <t-tooltip
                  :content="$t('organization.share.unshareAction')"
                  placement="top"
                >
                  <t-button
                    theme="danger"
...
import {
  ...
  listMCPShares,
  removeMCPShare,
  updateMCPSharePermission,
  type MCPServiceShare,
...
function getOrgForShare(organizationId: string) {
  return orgStore.organizations.find((org) => org.id === organizationId)
}
...
const shares = ref<(MCPServiceShare & { organization_name?: string })[]>([])

const availableOrganizations = computed(() => {
  const sharedOrgIds = new Set(shares.value.map((share) => share.organization_id))
...
const shareColumns = computed(() => {
  const cols = [
    {
      colKey: 'space',
      title: t('organization.share.columns.space'),
      ellipsis: true,
      minWidth: 180,
    },
    {
      colKey: 'permission',
      title: t('organization.share.columns.permission'),
      width: 132,
    },
    {
      colKey: 'created_at',
      title: t('organization.share.columns.sharedAt'),
      width: 154,
    },
  ]
  if (props.canShare) {
    cols.push({
      colKey: 'actions',
      title: t('organization.share.columns.operations'),
      width: 72,
      align: 'left',
...
async function loadShares() {
  ...
  try {
    const result = await listMCPShares(props.serviceId)
    if (result.success && result.data) {
      const sharesData =
        (result.data as { shares?: MCPServiceShare[] }).shares || result.data
      const sharesList = Array.isArray(sharesData) ? sharesData : []
      shares.value = sharesList.map((share: MCPServiceShare) => ({
        ...share,
        organization_name:
          share.organization_name ||
          orgStore.organizations.find((org) => org.id === share.organization_id)
            ?.name ||
          share.organization_id,
      }))
    }
  } catch (error) {
    console.error('Failed to load MCP shares:', error)
  } finally {
...
async function handleShare() {
  ...
  try {
    const result = await shareMCPService(props.serviceId, {
    ...
    const message =
      error instanceof Error ? error.message : t('organization.share.shareFailed')
...
async function handleUpdatePermission(
  share: MCPServiceShare,
  newPermission: string,
) {
  if (share.permission === newPermission) return

  try {
    const result = await updateMCPSharePermission(props.serviceId, share.id, {
    ...
    const message =
      error instanceof Error
        ? error.message
        : t('organization.roleUpdateFailed')
    MessagePlugin.error(message)
  }
}

async function handleUnshare(share: MCPServiceShare) {
  try {
    const result = await removeMCPShare(props.serviceId, share.id)
    if (result.success) {
    ...
    const message =
      error instanceof Error
        ? error.message
        : t('organization.share.unshareFailed')
    MessagePlugin.error(message)
  }
}
