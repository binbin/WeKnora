export default {
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      settings: {
        ...
        desc: 'Откройте это меню, чтобы управлять аккаунтом и системными настройками. Обучение можно снова открыть кнопкой помощи рядом с именем вверху меню.',
      },
      models: {
  ...
  agent: {
    ...
    sections: {
      builtin: 'Встроенные',
      mine: 'Созданные мной',
      tenantReadonly: '{name} · Только просмотр',
...
/example\.(net|org)/`,
  ...
  auth: {
    ...
    usernamePlaceholder: 'Введите 2–20 китайских иероглифов',
    ...
    usernameMinLength: 'Имя должно содержать не менее 2 китайских иероглифов',
    usernameMaxLength: 'Имя не может превышать 20 китайских иероглифов',
    usernameInvalid: 'Имя должно состоять только из 2–20 китайских иероглифов',
  ...
  embedPublish: {
    ...
    originsHint: 'Один полный origin в строке (напр. https://shop.example.com); пустое значение снимает ограничение. Поддерживаются поддомены *.example.com',
    ...
    publishTokenWarning: 'Токен публикации виден в исходном коде страницы встраивания — любой посетитель сайта может его прочитать. Считайте его публикуемым ключом, а не секретом: не встраивайте агентов с чувствительными данными, рекомендуется whitelist доменов и лимиты, при утечке немедленно смените токен.',
  ...
  knowledgeEditor: {
    ...
    basic: {
      ...
      shareWithDescendantsTip:
        'Если включено, дочерние организации могут читать эту базу знаний только для чтения. По умолчанию выключено.',
    },
    wiki: {
  ...
  resourceOrigin: {
    ...
    tenantTooltip: 'Создано другим участником организации',
  ...
  integrations: {
    ...
    chrome: {
      ...
      subtitle:
        'Для self-hosted TreeRAG: вопросы в боковой панели, клиппинг страниц и Markdown-заметки в базы знаний прямо при просмотре.',
    ...
    claw: {
      title: 'TreeRAG Skill',
      subtitle:
        'Импорт документов и гибридный поиск (вектор + ключевые слова) через REST API TreeRAG — загрузки, URL, Markdown и поиск.',
  ...
  orgUnit: {
    sectionDescription:
      'Дерево организаций платформы: корневые единицы создаёт системный администратор. Участники автоматически попадают в пространство с именем организации и делят его с пользователями дочерних единиц.',
    sectionDescriptionScoped:
      'Управление начинается с вашей организации: можно добавлять и менять только подчинённые единицы. Предки и соседние узлы недоступны.',
  },
  tenantMember: {
    title: 'Участники',
