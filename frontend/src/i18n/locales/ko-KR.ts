export default {
  ...
  settings: {
    ...
    parser: {
      ...
      paddleocrVlApiKeyPlaceholder: '자체 호스팅 API Key (선택)',
      paddleocrVlApiKeyHint: '서비스에 인증이 필요하면 Authorization: Bearer <key> 로 전송합니다. 공식 PaddleX는 비워 두세요.',
  ...
  auth: {
    ...
    usernamePlaceholder: "한자 2-20자 이름을 입력하세요",
    ...
    usernameMinLength: "이름은 한자 최소 2자여야 합니다",
    usernameMaxLength: "이름은 한자 20자를 초과할 수 없습니다",
    usernameInvalid: "이름은 한자 2-20자만 지원합니다",
  ...
  embedPublish: {
    ...
    originsLabel: '허용 도메인(한 줄에 하나, 비우면 제한 없음)',
    originsHint: '한 줄에 하나의 전체 출처(예: https://shop.example.com); 비우면 출처를 제한하지 않습니다. *.example.com 서브도메인 와일드카드 지원',
    ...
    publishTokenWarning: '게시 토큰은 임베드 페이지 소스에 평문으로 노출됩니다 — 호스트 사이트 방문자가 읽을 수 있습니다. 기밀 키가 아닌 공개 가능 키로 취급하세요: 민감 데이터 에이전트는 임베드하지 말고, 도메인 화이트리스트와 요청 상한 설정을 권장하며, 유출 시 즉시 토큰을 교체하세요.',
    debug: '디버그 미리보기',
  ...
  knowledgeEditor: {
    ...
    basic: {
      ...
      shareWithDescendantsLabel: "하위 기관에 공유",
      shareWithDescendantsTip:
        "선택하면 하위 기관이 이 지식베이스를 읽기 전용으로 참조할 수 있습니다. 기본값은 비공유입니다.",
    },
    wiki: {
      title: "Wiki 설정",
  ...
  agentEditor: {
    ...
    workspace: {
      ...
      backToAll: 'All',
      debugTitle: '디버그 미리보기',
      debugHint: '저장된 구성으로 테스트합니다. 변경 후 저장한 뒤 디버그하세요.',
      debugNeedSave: '에이전트를 저장하면 디버그할 수 있습니다',
      debugEmpty: '질문을 입력해 디버그를 시작하세요',
      debugPlaceholder: '테스트 질문 입력…',
      debugSend: '전송',
      debugClear: '세션 비우기',
      debugYou: '나',
      debugAgent: '에이전트',
      ...
      logsLoadFailed: '대화 로그를 불러오지 못했습니다',
      logsSourceWeb: '웹',
      logsSourceEmbed: '임베드',
    },
    publish: {
      types: {
        guest: '로그인 없는 창',
        guestDesc: '단축 링크로 바로 열기, 로그인·외부 사이트 임베드 불필요',
        embed: '웹 임베드',
        embedDesc: 'iframe 또는 위젯으로 기존 사이트에 삽입',
        api: 'API 접근',
        apiDesc: 'API로 기존 시스템에 연결',
        feishu: 'Feishu 봇',
        feishuDesc: 'API로 Feishu 봇 직접 연동',
        dingtalk: 'DingTalk 봇',
        dingtalkDesc: 'API로 DingTalk 봇 직접 연동',
        wechat_oa: '위챗 공식계정',
        wechat_oaDesc: '관리자 QR 인증으로 팬 채팅 인수',
        wecom: '기업 위챗',
        wecomDesc: '기업 위챗 봇 또는 자체 앱 연동',
        wechat: '개인 위챗',
        wechatDesc: 'iLink QR로 개인 위챗(1:1) 바인딩',
        portal: '포털',
        portalDesc: '포털 로그인 후 앱과 대화',
      },
      createLink: '새 링크 만들기',
      createBot: '봇 만들기',
      startUse: '시작하기',
      ...
      colApiKey: 'API KEY',
      colCreatedAt: '생성 시간',
      embedEmpty: '웹 임베드 채널이 없습니다',
      apiKeysTitle: 'API 키 관리',
      viewDocs: '문서 보기',
      apiDocTitle: '게시 채널 API 접근 안내',
      apiDocSubtitle: '게시 채널 API 키로 OpenAI 호환 대화 API를 호출합니다',
      apiDocBack: '뒤로',
      apiRoot: 'API 루트',
      apiRootHint: '루트는 엔드포인트가 아닙니다. /chat/completions 를 이어 붙이세요',
      apiAuthHint: 'Authorization: Bearer <key> 를 사용하세요',
      apiHint: '이 키는 현재 에이전트에 묶여 있으며 OpenAI 호환 대화 API에만 사용할 수 있습니다',
      apiEmpty: 'API 키가 없습니다',
      apiCurlTitle: '호출 예시',
      apiPlaintextTitle: 'API 키를 저장하세요',
      apiPlaintextWarn: '키는 한 번만 표시됩니다. 지금 복사하세요. 다시 볼 수 없습니다.',
      apiRevoke: '폐기',
      apiRevokeConfirm: '키 「{name}」을(를) 폐기할까요? 즉시 무효화되며 복구할 수 없습니다.',
      apiRevokeSuccess: 'API 키가 폐기되었습니다',
      apiRevokeFailed: 'API 키 폐기에 실패했습니다',
      imEmpty: '이 플랫폼의 봇 채널이 없습니다',
      portalDesc: '포털은 로그인 후 웹 대화 입구입니다. 조직 공유는 앱 설정에서 관리하세요',
      gotoShare: '공유 관리',
    },
    navGroups: {
      basic: '기본',
  ...
  integrations: {
    ...
    api: {
      ...
      signedFlowDetail:
        "HMAC secret은 TreeRAG와 신뢰할 수 있는 백엔드에만 보관하세요. 요청에 넣거나 브라우저에 배포하지 마세요. 요청 헤더에는 secret이 아니라 해당 secret으로 서명한 JWT 문자열을 넣습니다(발급마다 또는 만료 후 변경). JWT에는 sub(외부 사용자 ID), tenant_id, aud=weknora, exp(최대 24시간)가 필요합니다.",
      ...
      hmacSecretDesc: "TreeRAG에 저장된 것과 동일한 secret. 백엔드에서 JWT 서명에만 사용하며 요청 헤더로 보내지 않습니다.",
    ...
    chrome: {
      title: "지식 관리 어시스턴트",
      subtitle:
        "자체 호스팅 TreeRAG와 함께: 사이드바에서 질문하고, 웹 페이지를 클리핑하며, Markdown 메모를 지식베이스에 저장하세요.",
    ...
    claw: {
      title: "TreeRAG Skill",
      subtitle:
        "TreeRAG REST API로 문서를 가져오고 하이브리드 검색(벡터+키워드)을 수행합니다. 파일/URL/Markdown 업로드 및 검색에 사용.",
  ...
  tenantInvitation: {
    ...
    shareLink: {
      ...
      description:
        "일회용 가입 링크를 생성합니다. 누군가 링크로 가입하면 자동으로 무효화됩니다. {days}일 후 만료되며, 목록에서 언제든지 취소할 수 있습니다.",
      generate: "링크 생성",
      resultTitle: "초대 링크가 생성되었습니다",
      resultBody: "아래 링크를 복사해 전달하세요. 가입이 완료되면 링크는 자동 무효화됩니다. 사용 전에는 목록에서 다시 복사하거나 취소할 수 있습니다.",
  ...
  dataCharts: {
    desc: "조직 계층 및 하위 데이터를 시각화하여 관리자가 리소스를 종합적으로 파악할 수 있습니다",
    orgUnits: "조직 노드",
    organizations: "공유 스페이스",
    members: "총 멤버 수",
    shares: "공유 리소스",
    ...
    roleDist: "역할 분포",
    kbDist: "지식베이스 분포",
    resourceRadar: "리소스 개요",
    depthDist: "계층 깊이 분포",
    shareHeatmap: "공유 리소스 히트맵",
    kbKnowledgeCount: "지식베이스 문서 수 Top 10",
    kbChunkCount: "지식베이스 청크 수 Top 10",
    kbTypeDist: "지식베이스 유형 분포",
    agentModeDist: "에이전트 모드 분포",
    agentKBCount: "에이전트 지식베이스 연결",
    agentFeatureDist: "에이전트 기능 사용 통계",
    kbShares: "지식베이스 공유",
    agentShares: "에이전트 공유",
    ...
    kbTypeFAQ: "FAQ",
    kbTypeWiki: "위키",
    agentModeQuick: "빠른 답변",
    agentModeSmart: "스마트 추론",
    agentModeOther: "기타",
    kbModeAll: "전체",
    kbCount: "개 지식베이스",
    featureWebSearch: "웹 검색",
    featureMultiTurn: "다중 턴",
    featureMCP: "MCP 도구",
    featureImage: "이미지 이해",
    featureUsage: "기능 사용",
    noData: "데이터 없음",
