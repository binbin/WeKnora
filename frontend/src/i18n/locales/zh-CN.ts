export default {
  ...
  knowledgeStages: {
    ...
    errorCode: {
      ...
      TASK_STUCK: "任务长时间无进展，已自动回收",
      TASK_STUCK_SUGGESTION: "解析任务超过阈值时间仍无心跳或队列记录，系统已将其标记为失败。请点击重试；若反复出现，请检查 DocReader、向量服务与任务队列负载。",
  ...
  inviteRegister: {
    ...
    username: "姓名",
    usernamePlaceholder: "请输入2-20个汉字的姓名",
    password: "设置密码",
  ...
  auth: {
    ...
    usernameRequired: "请输入姓名",
    usernameMinLength: "姓名至少2个汉字",
    usernameMaxLength: "姓名不能超过20个汉字",
    usernameInvalid: "姓名仅支持2-20个汉字",
  ...
  embedPublish: {
    title: "网页发布",
    description: "生成可直接打开的短链接，或将智能体嵌入第三方网页（iframe / 浮窗）。",
    create: "新建发布渠道",
    channelsTitle: "发布渠道",
    disabled: "已停用",
    empty: "暂无嵌入渠道",
    ...
    originsHint: "每行一个完整来源地址（如 https://shop.example.com）；留空表示不限制来源。支持 *.example.com 子域通配",
    ...
    publishTokenWarning: "发布 Token 会以明文出现在嵌入页面源码中，任何访问该网站的人都能看到，请将其视为「可公开密钥」而非机密：切勿把含敏感数据的智能体做成嵌入渠道；建议配置域名白名单与请求上限；Token 泄露时可随时「重置 Token」作废旧值。",
    debug: "调试预览",
    ...
    webhookSecretPlaceholder: "用于 HMAC-SHA256 签名（X-TreeRAG-Signature）",
    ...
    deployAfterSaveHint: "保存渠道后，可在此复制短链接、渠道密钥与嵌入代码。",
  ...
  guestLinkPublish: {
    ...
    allowFileUpload: "显示文件上传",
    allowFileUploadDesc: "开启后，访客可上传图片与文档附件。仅当绑定的智能体已启用图片上传时才会生效。",
    allowFileUploadFollowAgent: "免登录短链的上传入口跟随智能体「图片上传」设置，无需再单独开关。",
    fileUploadFollowOn: "已跟随启用",
    fileUploadFollowOff: "智能体未启用",
    agentImageUploadDisabledHint: "当前智能体未启用图片上传，访客端不会显示上传按钮。请先在智能体设置中开启图片上传。",
    pageTitle: "页面标题",
    pageTitlePlaceholder: "AI 客服",
    defaultLocale: "默认语言",
  ...
  agentEditor: {
    ...
    publish: {
      ...
      apiHint: "这些密钥绑定当前智能体，仅可用于本智能体的 OpenAI 兼容对话接口",
      apiEmpty: "暂无 API 密钥",
      apiCurlTitle: "调用示例",
    ...
    im: {
      ...
      wechatOaBinding: "授权中...",
      wechatOaNeedCloud: "请先配置 TreeRAG Cloud 凭证与公网回调地址",
  ...
  integrations: {
    ...
    api: {
      ...
      signedFlowDetail:
        "HMAC 密钥仅保存在 TreeRAG 与你的可信后端，切勿放入请求或下发给浏览器。请求头里传的是用该密钥签出的 JWT 字符串（每次签发或过期后会变），不是密钥本身。JWT 须包含 sub（外部用户 ID）、tenant_id、aud=weknora、exp（有效期不超过 24 小时）。",
    ...
    agentEditor: {
      label: "发布渠道",
      desc: "生成可直接打开的短链接，或嵌入到第三方网站",
      manage: "管理",
    },
    chrome: {
      title: "知识管理助手",
      subtitle:
        "配合 TreeRAG 自建服务使用：在任意网页侧边栏提问、剪藏内容、Markdown 速记，将浏览中的知识沉淀到你的知识库。",
    ...
    claw: {
      title: "TreeRAG Skill",
      subtitle:
        "通过 TreeRAG REST API 导入文档并执行混合检索（向量 + 关键词）。适用于上传文件/URL/Markdown 到知识库、跨库检索与浏览知识内容。",
  ...
  tenantMember: {
    ...
    errors: {
      ...
      cannotManagePeerAdmin: "不能管理同级的其他管理员。",
      cannotPromotePeerAdmin: "不能将同级成员提升为管理员。",
      generic: "操作失败，请稍后重试。",
    },
    tabs: {
      members: "成员",
  ...
  tenantInvitation: {
    ...
    shareLink: {
      ...
      description:
        "生成一条一次性注册链接；有人通过链接加入后自动失效。也可在到期前手动撤销，或过期后失效。",
      generate: "生成链接",
      resultTitle: "邀请链接已生成",
      resultBody: "复制下方链接发给被邀请人。有人加入后链接会自动失效；未使用前也可在列表中重新复制或撤销。",
      revokeConfirm: "撤销后，未注册的人将无法再使用此链接；如需重发请生成新链接。",
    },
    revoke: {
      button: "撤销",
      confirmTitle: "确认撤销邀请？",
