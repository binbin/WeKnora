export default {
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: "每次在当前 TreeRAG 主机启动一个短生命周期容器执行脚本",
        local: "直接在 TreeRAG 服务进程所在环境执行脚本"
      },
      addConfig: "添加沙箱后端",
      ...
      disableScriptsConfirm: "禁用后，本空间内所有智能体将无法在沙盒中执行技能脚本；仍可阅读技能内容。已在运行中的远端沙盒不会自动销毁，需结束或删除相关会话后才会释放。是否继续？",
      ...
      localRuntimeWarning: "本地进程会直接在 TreeRAG 服务器上执行技能脚本，不具备容器或 MicroVM 隔离。仅建议在可信的开发空间使用。",
      ...
      templateStepHint: "集群缺少标准模板时，TreeRAG 会自动创建；模板就绪后才能继续。",
  ...
  system: {
    ...
    globalSettings: {
      ...
      passwordReset: {
        label: "重置用户密码",
        description: "为平台上任意已注册用户设置新密码（按邮箱）。重置成功后，该用户当前所有登录会话都会失效，需要使用新密码重新登录。",
        action: "重置密码",
  ...
  tenantInvitation: {
    ...
    shareLink: {
      ...
      description: "生成一条一次性注册链接；有人通过链接加入后自动失效。也可在到期前手动撤销，或过期后失效。",
      generate: "生成链接",
      resultTitle: "邀请链接已生成",
