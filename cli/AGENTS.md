...
This is the TreeRAG CLI (`weknora`), a command-line client for the TreeRAG RAG server. The module path is `github.com/Tencent/WeKnora/cli`.
...
Five design decisions readers may want context on: where TreeRAG picks an
...
| | |
|---|---|
| **TreeRAG** | success envelope → stdout; error envelope → stderr |
...
| | |
|---|---|
| **TreeRAG** | DELETE triggers exit-10 (`input.confirmation_required`); user bypasses with `-y/--yes` |
...
| | |
|---|---|
| **TreeRAG** | two separate fields: `retry_command` (suggested next argv, directly-executable for non-destructive errors; informational only on exit-10) + `hint` (prose) |
...
| | |
|---|---|
| **TreeRAG** | streaming commands (`chat`, `session ask`) emit bare `{type:...}` per line; no envelope |
...
| **TreeRAG** | version identity via CLI binary semver + CHANGELOG `### BREAKING` + skill `tested_against` + CI parity tests |
...
- Consistent with every existing non-auth TreeRAG command.
