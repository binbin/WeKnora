...
func TestStripAtMentionBasic(t *testing.T) {
	tests := []struct {
		...
		{"multi-word bot double-space", "@TreeRAG Bot  /stop", "/stop"},
		{"multi-word bot double-space chinese", "@TreeRAG Bot  什么是上下文工程", "什么是上下文工程"},
		{"multi-word bot single-space command", "@TreeRAG Bot /stop", "/stop"},
		{"multi-word bot single-space chinese", "@TreeRAG Bot 什么是老登", "什么是老登"},
...
func TestLongConnClient_StripAtMention(t *testing.T) {
	t.Run("learns bot name from double-space then handles single-space", func(t *testing.T) {
		...
		// First message: double-space → learn "TreeRAG Bot"
		got := c.stripAtMention("@TreeRAG Bot  什么是上下文工程")
		if got != "什么是上下文工程" {
			t.Errorf("first message: got %q, want %q", got, "什么是上下文工程")
		}

		// Verify bot name was cached
		if name, _ := c.botDisplayName.Load().(string); name != "TreeRAG Bot" {
			t.Errorf("cached bot name = %q, want %q", name, "TreeRAG Bot")
		}

		// Second message: single-space → should use cached name
		got = c.stripAtMention("@TreeRAG Bot /stop")
		if got != "/stop" {
			t.Errorf("second message: got %q, want %q", got, "/stop")
		}

		// Single-space with chinese content
		got = c.stripAtMention("@TreeRAG Bot 什么是B+树")
		if got != "什么是B+树" {
	...
	t.Run("preconfigured bot name", func(t *testing.T) {
		c := &LongConnClient{}
		c.botDisplayName.Store("TreeRAG Bot")
	...
	t.Run("cold start single-space chinese", func(t *testing.T) {
		...
		got := c.stripAtMention("@TreeRAG Bot 什么是老登")
		if got != "什么是老登" {
	...
	t.Run("cold start all-ascii content falls back to first word", func(t *testing.T) {
		...
		got := c.stripAtMention("@TreeRAG Bot hello")
		if got != "Bot hello" {
