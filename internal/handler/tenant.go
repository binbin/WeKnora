...
func (h *TenantHandler) CreateTenant(c *gin.Context) {
	...
	// Workspaces are provisioned by the platform (org-tree auto-create or
	// platform API keys). No interactive user — including cross-tenant
	// superusers — may self-create a workspace via this endpoint.
	if !platformCaller {
		logger.Warnf(ctx, "Tenant creation denied for user %s (self-service disabled)", caller.ID)
		c.Error(errors.NewTenantCreationDisabledError())
		return
	}

	var tenantData types.Tenant
	// Platform API key path: accept the full Tenant payload.
