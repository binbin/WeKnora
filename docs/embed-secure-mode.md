...
| 名称 | 格式 | 谁持有 | 用途 |
|------|------|--------|------|
| 发布 Token | `em_…` | 仅你的服务端 | 向 TreeRAG 换取短时令牌；在管理端「渠道密钥」查看 |
| 会话 Token | `ems_…` | 访客浏览器（iframe 内） | 调聊天、上传等 embed API；约 30 分钟过期，Widget 会自动刷新 |

## 工作流程

```
访客浏览器                 你的后端（shop 的服务器）              TreeRAG
     │                              │                              │
     │ 1. 加载 Widget               │                              │
     │    data-token-endpoint       │                              │
     │    （不含 em_）               │                              │
...
| 取令牌接口 **502** `mint failed` | TreeRAG 不可达、渠道已停用，或 exchange 响应格式不对 |
