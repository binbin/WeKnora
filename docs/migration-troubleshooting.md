...
This guide is linked from the system info page when TreeRAG's startup database
...
TreeRAG auto-runs `golang-migrate` migrations on every startup. When a
...
Then restart TreeRAG. The next startup will pick up where the failing
migration left off.
...
default, TreeRAG's startup tries to auto-recover; if you disabled that with
...
4. **Fix the underlying cause** (install extension, fix privileges, free
   disk, …), then either:
   - Restart TreeRAG and let auto-recovery retry; **or**
...
- TreeRAG version + commit ID (from the system info page).
