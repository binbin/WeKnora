...
func checkUpdate(ctx context.Context, currentVersion string, showUpToDate bool, autoDownload bool) {
	go func() {
		...
		req, err := http.NewRequest("GET", "https://api.github.com/repos/Tencent/TreeRAG/releases/latest", nil)
		if err != nil {
		...
		req.Header.Set("User-Agent", "TreeRAG-Lite-Desktop-App")
		...
		if showUpToDate {
			wailsruntime.MessageDialog(ctx, wailsruntime.MessageDialogOptions{
				Type:          wailsruntime.InfoDialog,
				Title:         "Up to Date",
				Message:       fmt.Sprintf("You are using the latest version of TreeRAG Lite.\n\nCurrent version: %s", currentVersion),
				Buttons:       []string{"OK"},
				DefaultButton: "OK",
			})
		}
...
`, savePath, execPath)
		...
		if strings.HasSuffix(strings.ToLower(savePath), ".dmg") {
			go func() {
				...
				mountPoint := filepath.Join(os.TempDir(), "TreeRAGUpdateMount")
