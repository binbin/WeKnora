...
DocReader 是 TreeRAG 项目中负责文档解析和处理的 gRPC 服务。它支持多种文档格式的读取、OCR 识别、多模态处理等功能。
...
- `MINIO_BUCKET_NAME`: MinIO 存储桶名称（默认：TreeRAG）
...
docker logs TreeRAG-docreader
```

## 更多信息

- 服务端口：50051（gRPC）
- 容器名称：TreeRAG-docreader
- 网络：TreeRAG-network
- 重启策略：unless-stopped
